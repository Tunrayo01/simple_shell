 ...
 */
int _my_exit(shellinfo_t *info)
{
	int exitcheck;

	if (info->argv[1])  /* If there is an exit arguement */
	{
		exitcheck = _erratoic(info->argv[1]);
		if (exitcheck == -1)
		{
			info->status = 2;
			prints_error(info, "Wrong number: ");
			_eputs(info->argv[1]);
			_eputchar('\n');
			return (1);
		}
		info->err_num = _erratoic(info->argv[1]);
		return (-2);
	}
	info->err_num = -1;
	return (-2);
